...
class ThompsonSampler:
    def __init__(self, n_bandits, id, alpha=0.5, beta=0.5, decay=0.99):
        ...
        try:
            self.wins = np.loadtxt(f'{self.id}/wins.txt')
            self.losses = np.loadtxt(f'{self.id}/losses.txt')
        except IOError:
    ...
    def update(self, chosen_bandit, df):
        ...
        np.savetxt(f'{self.id}/wins.txt', self.wins)
        np.savetxt(f'{self.id}/losses.txt', self.losses)
