...
def LTS(sampler, data, sample_size, filter_label, trainer, labeler, filename, balance, metric, baseline, labeling, indx, id, state_path):
    ...
    if improved:
        ...
        model_name = f"data/{id}/models/fine_tunned_{indx}_bandit_{chosen_bandit}"
        ...
        name = f'data/{id}/{filename}_training_data'
...
def add_previous_data(df, id):
    if os.path.exists(f'data/{id}/positive_data.csv'):
        pos = pd.read_csv(f'data/{id}/positive_data.csv')
        ...
        os.remove(f'data/{id}/positive_data.csv')
    return df
...
def save_bendit_results(filename, chosen_bandit, results, id):
    if os.path.exists(f'data/{id}/{filename}_model_results.json'):
        with open(f'data/{id}/{filename}_model_results.json', 'r') as file:
    ...
    with open(f'data/{id}/{filename}_model_results.json', 'w') as file:
